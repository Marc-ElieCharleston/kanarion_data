...
- **CI** : `scripts/validate_cross_references.py` étape 6 valide que tout `recipes.json` `output_item` existe (sinon item fantôme : id brut affiché, icône vide, pas de stats). Étape 7 : les 5 bandes de rang C→SS sont **recopiées dans 3 fichiers** (`panoplies.json _meta.rank_system`, `uniques.json _meta.rank_system`, `equipment_scaling.json tier_system.tiers`), chacun lu par des consommateurs distincts et aucun ne lisant celui d'un autre. L'étape vérifie que bornes, `rank_label` et `stat_multiplier` concordent — le vrai risque n'est pas le désordre mais la dérive silencieuse (écart de multiplicateur invisible en test, rang non résolu côté client). ⚠️ `recipes.json` a un **homonyme** : `_meta.craft_ranks` (ex-`rank_system`, renommé 2026-08-13) = rangs de **craft** par bandes de 10 niveaux, sémantique incompatible — l'étape 7 échoue si le nom `rank_system` y revient. Le rangement propre (un `_meta/item_ranks.json` unique) reste à faire : il touche 6 lecteurs sur 2 dépôts, la garde CI rend son absence sans danger en attendant.
...
- **`roster_status` (2026-08-02)** : `active` (défaut) = le monstre est dans le roster jouable ; `shelved` = parqué hors rotation (scaffolding endgame lv50-100 + échelle bosses Fissure pas encore dessinée). Le layer de spawn/presence ne sort QUE les `active`. **`art_pending: true`** = le sprite n'existe pas encore côté client (`kanarion_front/assets/monsters/`), tourne sur le fallback silhouette grise. **État courant : 107 active(art ok) + 16 active(art_pending, dont chaîne Edric = Acte 1 prioritaire) + 54 shelved.** Rien n'est supprimé (préserve refs quêtes/donjons + lore). Voir mémoire `project_monster_affix_refactor`. Le système préfixe/suffixe (affixe) sera rebâti proprement plus tard sur cette base saine.
