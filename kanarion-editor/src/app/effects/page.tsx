...
export default function EffectsReferencePage() {
  ...
  const navItems = [
    {
      id: 'all',
      icon: '📋',
      label: locale === 'fr' ? 'Toutes les Catégories' : 'All Categories',
  ...
  const filters = [
    {
      ...
      onChange: (value: string) => setFilterPolarity(value as 'all' | 'buff' | 'debuff'),
    },
  ];
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
      <SecondarySidebar
        title={locale === 'fr' ? 'Effets de Statut' : 'Status Effects'}
        subtitle={`${effects?._summary?.total || allEffects.length} effects • v${effects?._meta?.version || 'N/A'}`}
        showSearch
        searchValue={searchTerm}
        onSearchChange={setSearchTerm}
        searchPlaceholder={locale === 'fr' ? 'Rechercher...' : 'Search effects...'}
        filters={filters}
        navItems={navItems}
      />
        ...
        <div className="p-4 md:p-6 lg:p-8">
                        ...
                        {categoryEffects.map(({ key, effect }) => (
                          <TableRow key={key}>
                            ...
                            <TableCell className="text-sm whitespace-normal">
                              {effect.formula && (
                                <code className="text-xs text-amber-400 block mb-1 break-all">{effect.formula}</code>
                              )}
          ...
          <Card className="mt-8 p-6 border-blue-500/30">
            <h3 className="text-xl font-semibold mb-4 flex items-center gap-2 text-blue-400">
              <span>📊</span> {locale === 'fr' ? 'Système de Stacking' : 'Stacking System'}
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div className="bg-zinc-800 p-4 rounded">
                <h4 className="font-medium text-blue-400 mb-2">Stackable</h4>
                <p className="text-sm text-zinc-400">+{config.stacking_system.value_per_stack}% {locale === 'fr' ? 'par stack' : 'per stack'}</p>
            ...
            <div className="bg-zinc-800 p-4 rounded">
              <h4 className="font-medium text-emerald-400 mb-2">{locale === 'fr' ? 'Exemples de Stacking' : 'Stacking Examples'}</h4>
          ...
          <Card className="mt-8 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-medium text-emerald-400 mb-3">{locale === 'fr' ? 'Durée des Buffs' : 'Buff Durations'}</h4>
              ...
              <div>
                <h4 className="font-medium text-red-400 mb-3">{locale === 'fr' ? 'Durée des Debuffs' : 'Debuff Durations'}</h4>
                <div className="space-y-2">
